...
export default function DealerDashboard(props) {
	...
	const onSubmitSecondForm = async (values) => {
		...
		await createMobileShipmentApi(values).then(res => {
			console.log("Res => ", res);
			localStorage.setItem("sarokhPay", JSON.stringify({trackingNo: res, senderName: values.senderName, totalAmount: values.totalAmount}));
			setShipmentId(res);
			createBillPayload = {
				...
				"shipmentsIdList": res,
				"startDate": new Date(),
				"deliveryCharges": values.paymentMethod !== "COD" ? values.totalAmount : 0,
										...
										<div className="card-body">
											...
											<div className="form-row">
												...
												<div className="col-sm-6">
													...
													<label className="col-sm-6 col-6">
														<p className=" text-left">{ firstFormValues.senderName }</p>
													</label>
												</div>
												<div className="col-sm-12 right-align">
													<label className="col-sm-6 col-6 redcolor">Amount:</label>
													<label className="col-sm-6 col-6 text-right">Sar { firstFormValues.totalAmount }/-</label>
												</div>
