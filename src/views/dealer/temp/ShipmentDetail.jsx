...
export default function DealerDashboard(props) {
	...
	const onSubmitSecondForm = async (values) => {
		...
		if(values.paymentMethod === "COD"){
			values.codAmount = values.totalAmount;
	...
	const onPaidClick = async () => {
		const recordBillPayload = {
			amountPaid: firstFormValues.paymentMethod === "COD" ? 0 : firstFormValues.totalAmount,
			billNo: billId,
			paymentDate: new Date(),
			paymentMethod: firstFormValues.paymentMethod,
								...
								<>
									<div className="card-header">
										{localStorage.getItem('Language') != 'Arabic' ? (
											...
											<h2 className="text-right pl-2" dir="rtl">إنشاء شحنة - تفاصيل الشحنة</h2>
										)}
									</div>
													...
													<Form>
														...
														{localStorage.getItem('Language') != 'Arabic' ? (
															...
															<button
																...
																className={
																	!(dirty && isValid)
																		? 'disabled-btn btn btn-info float-left btnbrown mt-2'
																		: 'btn btn-info float-left btnbrown mt-2'
																}
																disabled={!(dirty && isValid)}
															>
																التالى
													...
													<Form>
																	...
																	<tr>
																		...
																		'Arabic' ? (
																			<td className="font14" align="right">
																				SAR{' '}
																				{
																					(firstFormValues.shipmentValue / 100) * shipperDeliveryCharges.insurance
																				}
																				/-
																			</td>
																		) : (
														...
														<button
															...
															{localStorage.getItem('Language') != 'Arabic' ? (
																...
																'btn btn-info float-left btn-danger  mt-2 ml-2'
															)}
														>
															{localStorage.getItem('Language') != 'Arabic' ? (
																<span>Go Back</span>
											...
											<div className="col-sm-12 right-align">
												...
												{localStorage.getItem('Language') != 'Arabic' ? (
													<label className="col-sm-6 col-6 text-right">
														Sar {firstFormValues.paymentMethod === "COD" ? "0" : firstFormValues.totalAmount}/-
													</label>
												) : (
													<label className="col-sm-6 col-6 text-right">
														صار {firstFormValues.paymentMethod === "COD" ? "0" : firstFormValues.totalAmount}/-
													</label>
												)}
											</div>
